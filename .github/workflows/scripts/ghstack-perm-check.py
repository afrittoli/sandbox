...
def main(check):
    ...
    with open(env_file, "a") as envfile:
        envfile.write(f"IS_GHSTACK=f{isgh}\n")
        envfile.write(f"REPO=f{chatops.REPO}\n")
        envfile.write(f"PR_NUMBER=f{chatops.NUMBER}\n")
        envfile.write(f"PR_REF=f{chatops.pr_ref}\n")
        envfile.write(f"PR_URL=f{chatops.PR_URL}\n")
